...
const DATASET_ID = 23;
const ids = ["工业-Q04","跨期-Q08","能源交通-Q06","能源交通-Q07","能源交通-Q05","跨期-Q03","核算-Q04","财政-Q04"];
...
for (const id of ids){
  ...
  ok?pass++:fail++;
  console.log((ok?"PASS":"FAIL")+" "+id+" metrics="+JSON.stringify(plan.metrics)+" ["+v.join(" ")+"]");
  console.log("   SQL: "+exec.data.sql);
  if (!ok) console.log("   rows:", JSON.stringify(rows.slice(0,3)), "warnings:", JSON.stringify(exec.data.dataWarnings));
}
