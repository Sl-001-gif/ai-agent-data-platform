...
- **数据规模**：`gov_info_record` 3601 条真实记录（邵阳市人民政府门户网站信息公开目录，0 重复 / 0 壳页 / 0 导航噪音），由 `tools/scraper/gov_scraper.py` 抓取入库。
- **分析链路**：政务关键词自动路由到政务计划（SALES_TREND / RANKING / STRUCTURE / GENERAL），规则模板 + 真实 LLM 双模式生成 SQL，经 9 条安全校验后受控执行，输出图表 / AI 解读 / 推荐追问 / 分析报告。
- **演示问题**：① 邵阳近 3 年按月发文量趋势（折线）② 各公开类目发文量分布占比（饼图）③ 各部门/单位发文量排名 Top10（柱状）。
- **发文单位补齐**：`publish_unit` 原全表为空，用 `python tools/scraper/backfill_unit.py` 按三级口径补齐——A 详情页真实单位（联网 `--mode detail`）、B 部门子站域名推断、C 类目代理兜底（近似推断，口径已标注）；幂等可重跑，`--dry-run` 预演，B+C 无需联网。
- **指标口径**：`metric_definition` 已修订——发文量、类目占比（分母=全表）、平均每日发文量（防除零、提示长区间稀释）、单位发文量（三级口径）。
